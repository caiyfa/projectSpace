...
public class BaseDAO  implements IBaseDAO {
     ...
     * @throws RunException
     */
    private JSONObject transferVOToJson(BaseVO baseVO) throws RunException {
     ...
     * @throws RunException
     */
    private BaseVO transferJsonToVO(Class<? extends  BaseVO> voClass, JSONObject jsonObject) throws RunException {
        if(jsonObject==null){
            return null;
        }
        try {
    ...
    private JSONArray transferVOListToJsonArr(List<? extends BaseVO> list) throws RunException {
    ...
    private List<BaseVO> transferJsonArrToVoList(Class<? extends BaseVO> cls,JSONArray jsonArray) throws RunException {
        try {
    ...
    public int updateVO(BaseVO baseVO) throws RunException {
        ...
        List<BaseVO> baseVOS= (List<BaseVO>) this.queryAllVO(baseVO.getClass());
        for(BaseVO vo:baseVOS){
            if(vo.getPrimaryKey().equals(baseVO.getPrimaryKey())){
                vo=baseVO;
            }
        }

        DataFileUtil.getInstance().curingData(baseVO.getClass().getSimpleName(),this.transferVOListToJsonArr(baseVOS));
        return 1;
    }
    @Override
    ...
    @Override
    public int deleteVOS(List<? extends BaseVO> voList) throws RunException {
        if(voList==null||voList.size()==0){
            return 0;
        }
        List<BaseVO> baseVOS= (List<BaseVO>) this.queryAllVO(voList.get(0).getClass());
        //若取得的数据为空则返回0
        if(baseVOS==null||baseVOS.size()==0){
            return 0;
        }
        ...
        for(int i=0,len=baseVOS.size();i<len;i++){
            for(BaseVO vo:voList){
                if(baseVOS.get(i).getPrimaryKey().equals(vo.getPrimaryKey())){
                    baseVOS.remove(i);
                    i--;
                    len--;
                    count++;
        ...
        DataFileUtil.getInstance().curingData(voList.get(0).getClass().getSimpleName(),this.transferVOListToJsonArr(baseVOS));
        return count;
    }
    @Override

...
import psn.base.utils.JsonUtil;
...
public class BaseDAO  implements IBaseDAO{
    ...
    public BaseVO insertVO(BaseVO baseVO) throws RunException {
        ...
        preparePK(baseVO);
        //获取已存在的数据
        JSONArray jsonArray= JsonUtil.getInstance().getJsonArr(baseVO.getClass().getSimpleName());
        //转换数据为JSON
        JSONObject jsonObject=this.transferVOToJson(baseVO);
        jsonArray.put(jsonObject);
        //固化数据到文件
        JsonUtil.getInstance().curingData(baseVO.getClass().getSimpleName(),jsonArray);
        return baseVO;
    }
    ...
    public int deleteVOByPK(Class<? extends BaseVO> cls, String pk) throws RunException {
        ...
        JsonUtil.getInstance().curingData(cls.getSimpleName(),this.transferVOListToJsonArr(baseVOS));
        return 1;
    }
    ...
    public  List<?> queryAllVO(Class<? extends BaseVO> cls) throws RunException {
        ...
        JSONArray jsonArray=JsonUtil.getInstance().getJsonArr(cls.getSimpleName());
